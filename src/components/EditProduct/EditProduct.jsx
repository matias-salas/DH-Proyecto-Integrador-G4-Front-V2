...
const EditPorduct = () => {
  ...
  useEffect(() => {
    ...
    .then((res) => res.json())
    .then((data) => {
      // Aquí estableces los estados con los datos del producto
      setPropertyName({ value: data.data.name});
      setAddress({ value: data.data.address });
      setDescription( data.data.description );
      setSitePolicy( data.data.policiesCancellation);
      setHealtAndSafetyPolicy( data.data.policiesSecurityAndHealth);
      setCancellationPolicy( data.data.policiesSite);
      // Repite esto para los demás estados
    })
  ...
  const uploadProduct = async () => {
    ...
    try {
      const response = await fetch(`${baseUrl.url}/products/update`, {
        method: 'PUT',
        headers: {
          'Access-Control-Allow-Origin': '*',
          Accept: 'application/json',
  ...
  return (
    <div className={style.container}>
      ...
      <form className={style.formProduct} onSubmit={handleSubmit}>
        ...
        <button className={`btn btn2 ${style.btnSubmit}`}>Confirmar</button>
      </form>
    </div>
  );
};
